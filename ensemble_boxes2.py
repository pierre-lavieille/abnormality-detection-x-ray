...
def get_weighted_box(boxes, conf_type='avg'):
    ...
    for b in boxes:
        ...
        w += b[2]
    box[0] = boxes[0][0]  #car toutes les boites sont du meme type
    if conf_type == 'avg':
...
def weighted_boxes_fusion(boxes_list, scores_list, labels_list, weights=None, 
                          iou_thr=0.55, skip_box_thr=0.0, conf_type='vote', 
    ...
    if conf_type not in ['avg', 'max', 'box_and_model_avg', 'absent_model_aware_avg', 'vote']:
    ...
    if len(overall_boxes) == 0 : 
        ...
        scores = overall_boxes[:, 1] / (sum(weights)*0.5)
